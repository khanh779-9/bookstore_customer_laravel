    ...
    <div className="space-y-6">
      ...
      <div className="admin-card">
        <div className="mb-6 relative max-w-sm">
          ...
          <input type="text" placeholder="Tìm tên hoặc email..." className="w-full pl-10 pr-4 py-2 border rounded-lg outline-none" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
          ...
          <tbody>
            {loading ? <tr><td colSpan="5" className="text-center py-8">Đang tải...</td></tr> : filtered.map(i => (
              <tr key={i.nhanvien_id}>
                ...
                <td>{i.email}</td>
                <td>
                  <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${i.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                    {i.role === 'admin' ? <FiShield /> : null} {i.role === 'admin' ? 'Quản trị' : 'Nhân viên'}
                  </span>
                </td>
                <td className="flex justify-center gap-2">
                  <button onClick={() => { setEditingItem(i); setFormData({ ho: i.ho, ten: i.ten, email: i.email, role: i.role, password: '' }); setIsModalOpen(true); }} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><FiEdit2 /></button>
                  <button onClick={() => handleDelete(i.nhanvien_id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><FiTrash2 /></button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="bg-white rounded-xl w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <input type="text" placeholder="Họ" required className="w-full border p-2 rounded" value={formData.ho} onChange={e => setFormData({...formData, ho: e.target.value})} />
                <input type="text" placeholder="Tên" required className="w-full border p-2 rounded" value={formData.ten} onChange={e => setFormData({...formData, ten: e.target.value})} />
              </div>
              ...
              <input type="password" placeholder={editingItem ? "Để trống nếu không đổi" : "Mật khẩu"} required={!editingItem} className="w-full border p-2 rounded" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
