...
export default function CustomToast({ message, type = "success", onClose, duration = 3000 }) {
  useEffect(() => {
    if (!duration) return;
  ...
  return (
    <motion.div
      ...
      className={cn(
        "fixed top-20 right-6 z-[9999] flex items-center gap-3 px-5 py-4 rounded-2xl border backdrop-blur-xl shadow-2xl min-w-[320px] max-w-md",
        colors[type]
      )}
      role="alert"
    >
