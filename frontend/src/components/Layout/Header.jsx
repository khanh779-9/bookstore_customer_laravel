        ...
        <div className="container mx-auto px-4 max-w-[1600px] flex items-center justify-between gap-4 md:gap-8">
          ...
          <div className="flex items-center gap-6 shrink-0">
            <Link to="/" className="flex items-center gap-2 group">
              <img
                src="/assets/images/bookstoreLogo.png"
                alt="Logo"
                className="h-9 w-auto object-contain"
              />
              <span className="hidden sm:block text-xl font-bold text-secondary tracking-tight">
                BookZone
              </span>
            </Link>
            ...
            <div
              ...
              ref={activeDropdown === "categories" ? dropdownRef : null}
            >
              <button
                onClick={() =>
                  setActiveDropdown(
                ...
                className="flex items-center gap-2 font-bold text-slate-600 hover:text-primary transition-colors text-sm cursor-pointer"
              >
          ...
          <div className="flex-1 max-w-2xl flex items-center gap-6">
            <form onSubmit={handleSearch} className="flex-1 max-w-md">
              <Input
                ...
                className="bg-slate-100 border-none py-2"
              />
            </form>
          ...
          <div className="flex items-center gap-1 sm:gap-3 shrink-0">
            ...
            {isAuthenticated && (
              <div
                className="relative"
                ref={activeDropdown === "notifications" ? dropdownRef : null}
              >
                <button
                  onClick={() =>
                    setActiveDropdown(
                ...
                <AnimatePresence>
                  {activeDropdown === "notifications" && (
                    ...
                    >
                      ...
                      <div className="max-h-80 overflow-y-auto">
                        {notifications.length > 0 ? (
                          notifications.slice(0, 5).map((n) => (
                            <div
                              key={n.id}
                              className="p-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-none"
                            >
                              <p
                                className={cn(
                                  "text-sm font-bold",
                      ...
                      <Link
                        to="/notifications"
                        className="block text-center p-3 text-xs font-bold text-primary hover:underline border-t border-slate-50"
                      >
                        Xem tất cả
                      </Link>
                    </motion.div>
                  ...
                  {activeDropdown === "user" && (
                    ...
                    >
                      <div className="p-3 border-b border-slate-50 mb-1">
                        ...
                        <p className="text-sm font-bold text-secondary truncate mt-1">
                          {user?.ten}
                        </p>
                      </div>
                      <UserMenuItem to="/account" icon={FiUser}>
                        Hồ sơ
                      </UserMenuItem>
                      <UserMenuItem to="/orders" icon={FiShoppingBag}>
                        Đơn hàng
                      </UserMenuItem>
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
                <Link
                  to="/"
                  className="p-3 text-lg font-bold text-secondary hover:text-primary"
                >
                  Trang chủ
                </Link>
                <Link
                ...
                <p className="px-3 text-[10px] font-bold text-slate-300 uppercase tracking-widest">
                  Danh mục
                </p>
                {categories.map((cat) => (
                  <Link
                    key={cat.id}
                    to={`/products?danhmucSP_id=${cat.id}`}
                    className="px-3 py-2 text-slate-600 font-bold text-sm"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {cat.name}
                  </Link>
              ...
              {!isAuthenticated && (
                <div className="pt-6 border-t border-slate-50 flex flex-col gap-3">
                  <Link to="/login" className="w-full btn-secondary py-3">
                    Đăng nhập
                  </Link>
                  <Link to="/register" className="w-full btn-primary py-3">
                    Tham gia ngay
                  </Link>
                </div>
              )}
...
function NavLink({ to, children }) {
  return (
    <Link
      to={to}
      className="text-sm font-bold text-slate-500 hover:text-primary transition-colors whitespace-nowrap"
    >
      {children}
    </Link>
  );
...
function UserMenuItem({ to, icon: Icon, children }) {
  return (
    <Link
      to={to}
      className="flex items-center gap-3 px-3 py-2 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-primary transition-all"
    >
